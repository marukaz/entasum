...
class Seq2Seq(Model):
    """
    This is a variant of SimpleSeq2Seq in AllenNLP. Docstring below is same as that one.
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for indices in predicted_indices:
            ...
            for ixs in indices:
                ...
                if self._end_index in ixs:
                    ixs = ixs[:ixs.index(self._end_index)]
                predicted_tokens = [self.vocab.get_token_from_index(x, namespace=self._target_namespace)
                                    for x in ixs]
                beam_tokens.append(predicted_tokens)
            all_predicted_tokens.append(beam_tokens)
